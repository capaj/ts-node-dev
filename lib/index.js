...
module.exports = function(script, scriptArgs, nodeArgs, opts) {
  ...
  function start() {
    ...
    ipc.on(child, 'error', function(m) {
      ...
      let lastRequire = m.lastRequire
      if (
        m.code === 'MODULE_NOT_FOUND' &&
        lastRequire &&
        lastRequire.path.startsWith('.')
      ) {
        const pathNoExt = path.normalize(
          path.join(path.dirname(lastRequire.filename), lastRequire.path)
        )
